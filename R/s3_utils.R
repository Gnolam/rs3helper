connect_test <- function(access_key_id, secret_access_key, region = NULL) {
  ...
  if(length(response) > 0) response else 'response is NULL'
}
...
lookup_locations <- function() {
  path <- system.file('python', 'lookup_locations.py', package = 'rs3helper')
  command <- paste('python', path)
  response <- system(command, intern = TRUE)
  if(length(response) > 0) fromJSON(response) else 'response is NULL'
}

lookup_regions <- function() {
  path <- system.file('python', 'lookup_regions.py', package = 'rs3helper')
  command <- paste('python', path)
  response <- system(command, intern = TRUE)
  if(length(response) > 0) fromJSON(response) else 'response is NULL'
}

lookup_bucket <- function(access_key_id, secret_access_key, bucket, region = NULL) {
...
get_all_buckets <- function(access_key_id, secret_access_key) {
