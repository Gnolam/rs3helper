...
The package is hosted in GitHub and it is possible to install/load it as following.
...
As the 4 arguments should be provided each time when one of the collection of functions is executed, a wrapper function called **rs3wrapper()** is created. It returns a function that has a function argumnet (*f*) and unspecified argument (*...*). Then it is possible to execute a function where connection related arguments are pre-filled. Below is the source code of the function.
...
An example of executing *lookup_bucket()* is shown below. Although the last two arguments can be skipped as they have default values, the authentication arguments must be provided each time. Besides there may be occasions when non-default values should be entered for them. Entering these argumens many times can be tedious and it is convenient that those are wrapped in *rs3wrapper()*. See more on its document (*?rs3wrapper*)
...
wrapper(lookup_bucket, bucket_name = 'rs3helper') # a bucket called rs3helper is created
...
In order to look up specific bucket or key, *lookup_bucket()* and *lookup_key()* can be used. Both of them return a list of lookup information.
...
Also more than one objects can be checked by *get_all_buckets()* and *get_keys()*. As the name suggests the former returns information of all buckets while the latter returns information of keys. *get_keys()* has an extra argument called *prefix* that filters keys to look up. For example, if a bucket has a folder named *json* and only the keys in the folder need to be checked, it is possible to set the argument's value to be *json*. Then only the keys in the folder are looked up - subfolders can also be prefixed (eg folder/subfolder). Both the functions return a data frame of lookup information.
...
*get_access_control_list()* has 2 extra arguments: *bucket_name* and *key_name*. If *key_name* is *NULL*, the access control list of the bucket is returned. Otherwise the specific key's access control list is returned.
...
By default, all objects are *private* and an extra level of permission can be set up. For example, the bucket has two extra rows which indicates it has *public-read-write* permission as well.
